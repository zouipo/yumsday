...
// MockUserService is a mock implementation of UserService for testing handler
...
func (m *MockUserService) addUser(user *model.User) {
	user.ID = m.nextID
	m.nextID++
	m.users = append(m.users, *user)
}

func createTestUser(id int64, username, password string) *model.User {
	avatar := enum.Avatar1
	return &model.User{
		ID:        id,
		Username:  username,
...
func compareUserToUserDto(userDto *dto.UserDto, user *model.User) error {
...
// returns an error if any field does not match.
func compareUserToNewUserDto(user *model.User, newUserDto *dto.NewUserDto) error {
...
func TestGetUserByID_Success(t *testing.T) {
	...
	ctx := context.WithValue(r.Context(), ctxkey.IdCtxKey{}, expected.ID)
	r = r.WithContext(ctx)
	w := httptest.NewRecorder()
...
func TestGetUserByID_NotFound(t *testing.T) {
	...
	ctx := context.WithValue(r.Context(), ctxkey.IdCtxKey{}, int64(invalidUserId))
	r = r.WithContext(ctx)
	w := httptest.NewRecorder()
...
func TestAuthMe_Success(t *testing.T) {
	...
	r = r.WithContext(context.WithValue(r.Context(), ctxkey.UserCtxKey{}, authenticatedUser))
	w := httptest.NewRecorder()
...
func TestAuthMe_InvalidUserTypeInContext(t *testing.T) {
	...
	r := httptest.NewRequest(http.MethodGet, "/user/me", nil)
	r = r.WithContext(context.WithValue(r.Context(), ctxkey.UserCtxKey{}, "not-a-user"))
	w := httptest.NewRecorder()
...
func TestCreateUser_ValidationError(t *testing.T) {
	...
	errMessage := "Validation error on field 'username': " + customErrors.USERNAME_FIELD_ERROR
...
func TestUpdateUser_Success(t *testing.T) {
	...
	avatar := enum.Avatar2
	user := mapper.ToUserDtoNoPassword(&mockService.users[0])
	user.Username = validUsername
	user.Avatar = &avatar
	user.Language = enum.French
	user.AppTheme = enum.Dark
...
func TestUpdateUser_ConflictError(t *testing.T) {
	...
	if err := compareUserToUserDto(mapper.ToUserDtoNoPassword(&mockService.users[0]), actual); err != nil {
		t.Error("Updated user has mismatched fields: " + err.Error())
	}
}
...
func TestUpdateUserAdminRole_Success(t *testing.T) {
	...
	rolePayload := dto.AdminRolePayload{AppAdmin: adminRole}
	...
	r.Header.Set(http_header.CONTENT_TYPE_HEADER, http_header.APPLICATION_JSON)
	ctx := context.WithValue(r.Context(), ctxkey.IdCtxKey{}, int64(user.ID))
	r = r.WithContext(ctx)
	w := httptest.NewRecorder()
...
func TestUpdateUserPassword_Success(t *testing.T) {
	...
	passwordPayload := dto.PasswordPayload{
...
func TestUpdateUserPassword_ValidationError(t *testing.T) {
	...
	errMessage := "Validation error on field 'password': " + customErrors.PASSWORD_FIELD_ERROR
	if !strings.Contains(w.Body.String(), errMessage) {
		t.Errorf("expected error message containing '%s' instead of '%s'", errMessage, w.Body.String())
	}
